...
for (person_offset = 0; person_offset < person_length/sizeof(Person); person_offset++) {
    ...
    //     // but person must like some of these other guys
    //     score1 = get_score(person, areltd);
    //     if (likes_artist(person, artist)) score1=-1;
    ...
    //         unsigned int knows_pos= knows_map[knows_offset];
    //         if(location_map[person_offset] != location_map[knows_pos]) continue;
    //         //    printf("diverse location\n");
            
    //         knows = &person_map[knows_map[knows_offset]];
    //         //if(location_map[knows_pos] !=knows->location)
    //         //    printf("diverse location knows\n");
